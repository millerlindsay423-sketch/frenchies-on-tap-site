...
- **Brewery photos for the other 31 breweries** — Cherry Street, Crooked Culture, Horned Owl, Lazy Guy, Reformation, and Schoolhouse now have real photos (`images/breweries/`). The remaining 31 brewery pages still show "photo pending" — send more photos + which brewery whenever you have them and Claude Code can add a `"photos"` array to that brewery's entry in `data/breweries.json` and rerun `node scripts/build-breweries.js`.
